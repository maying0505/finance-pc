...
class Main extends React.Component {
    ...
    columns = [
        ...
        },
        {
            title: '催收类型',
            dataIndex: 'type',
            render: (text) => <span>{this.typeShow(text)}</span>
        },
        {
            title: '投诉内容',
            dataIndex: 'content',
    ...
    render() {
        ...
        const {type, loading, data, statusArr, typeArr, pagination, visible, id, ItemArr, httpUrl, title} = this.state;
        return (
                ...
                <Spin size='large' spinning={loading}>
                    ...
                    <Form onSubmit={this._handleSubmit}>
                        <Row>
                            ...
                            <Col {...ColConfig}>
                                <Form.Item {...FormItemLayout} label='投诉类型' style={{marginBottom: '0px'}}>
                                    {getFieldDecorator('type', {
                                        rules: [{required: false, message: '请选择'},
                                        ],
                                    })(
                                        <Select placeholder='请选择' allowClear={allowClear}>
                                            {typeArr.map(i => <Option value={i.id} key={i.id}>{i.name}</Option>)}
                                        </Select>
                                    )}
                                </Form.Item>
                            </Col>
