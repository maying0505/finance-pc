...
import './OfflinePayRecordList.less';
...
import AddOfflinePay from './AddOfflinePay';
...
class Main extends React.Component {
    ...
    _fetchData = async (pageNum, queryObj = this.state.queryObj) => {
        try {
            ...
            const {code, message: msg, data} = await Http.offlinePayRecord(params);
    ...
    _channel = async () => {
        try {
            const {code, message: msg, data} = await Http.financeTransChannel({type: 0});
            if (code === 200) {
    ...
    _handleExport = async () => {
        try {
            ...
            const {code, message: msg, data} = await Http.offlineRecordExport({...params});
            if (code === 200) {
    ...
    _handleVisible = (stateValue,reset) => {
    ...
    render() {
        ...
        const {statusArr, loading, data, pagination, visible, loanStatus, allLoanProduct} = this.state;
        const {clientHeight} = document.body;
        const tableHeight = clientHeight - 150;
        return (
            <div className='offline-pay-record-list'>
                        ...
                        <Form.Item style={{marginBottom: '10px'}}>
                            <div className='button-view'>
                                ...
                                <Button
                                    ...
                                    className="close"
                                    htmlType="button"
                                    onClick={() => this._handleVisible(true)}
                                >
                                    新增线下打款
                                </Button>
                                <Button
